...
public class AsmTypes {
    ...
    private static final List<Type> OPTIMIZED_CALLABLE_REFERENCE_SUPERTYPES =
            CollectionsKt.flatten(Arrays.asList(
                    Collections.singletonList(FUNCTION_REFERENCE_IMPL),
                    Arrays.asList(PROPERTY_REFERENCE_IMPL),
                    Arrays.asList(MUTABLE_PROPERTY_REFERENCE_IMPL)
            ));

    public static boolean isOptimizedCallableReferenceSupertype(@NotNull Type type) {
        return OPTIMIZED_CALLABLE_REFERENCE_SUPERTYPES.contains(type);
    }
