...
object Aggregates : TemplateGroupBase() {
    ...
    val f_scan = fn("scan(initial: R, operation: (acc: R, T) -> R)") {
        ...
        val returnType = if (f == Sequences) "sequence" else "list"

        doc {
            """
            Returns a $returnType containing successive accumulation values generated by applying [operation] from left to right 
